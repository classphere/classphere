...
import { createDepartmentMember, deactivateDepartmentMember, getReviewPaper, listDepartmentMembers, listReviewPapers, transitionReviewPaper, updateReviewQuestion, validatePaper, uploadImage } from "./test-department.controller";
...
router.get("/papers/:id/validate", authenticate, requireRole("test_department_head", "test_department_member"), validatePaper);
...
router.post("/upload", authenticate, requireRole("test_department_head", "test_department_member"), upload.single("image") as any, uploadImage);
