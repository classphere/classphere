...
/**
 * SSC Tier 1 has 4 sections of exactly 25 questions each.
 * Official time: 60 minutes total, but each section is conceptually 15 minutes.
 * These are the optimal time splits per section for SSC CGL/CHSL Tier 1.
 */
const OPTIMAL_TIME_SPLIT_PCT: Record<string, Record<string, number>> = {
  "ssc-cgl":  { "General Intelligence & Reasoning": 25, "General Awareness": 25, "Quantitative Aptitude": 25, "English Comprehension": 25 },
  "ssc-chsl": { "General Intelligence & Reasoning": 25, "General Awareness": 25, "Quantitative Aptitude": 25, "English Comprehension": 25 },
  "ssc-mts":  { "General Intelligence & Reasoning": 25, "General Awareness": 25, "Quantitative Aptitude": 25, "English Comprehension": 25 },
  "ssc-gd":   { "General Intelligence & Reasoning": 25, "General Awareness": 25, "Quantitative Aptitude": 25, "English Comprehension": 25 },
};

// Default when SSC variant is unknown
...
export function analyzeAttemptStrategy(
  ...
  totalTestDurationSec: number = 3600, // 60 minutes for SSC Tier 1
...
function buildInsight(
  ...
  if (score >= 80) {
    ...
    const examAdvice = examType.startsWith("ssc")
      ? "For SSC: complete each section in one sweep, then use remaining time to revisit GK questions."
      : "For SSC: complete each section in one sweep, then use remaining time to revisit GK questions.";
    recommendation = `Try the 2-round sweep strategy: first pass — answer all easy and medium questions. ` +
      `Second pass — attempt harder questions with remaining time. ${examAdvice}`;
  } else {
