...
// ── NEET Exam Date ───────────────────────────────────────────────────────────
// NEET UG is held in May (anchored to first Sunday of May, approx May 4)
const NEET_DATE = { name: "NEET UG", month: 5, day: 4 };  // May 4

/**
 * Computes the next upcoming NEET exam date from today and derives urgency mode.
 * Only handles NEET exam codes — JEE is handled in the JEE pipeline.
 */
export function getExamCountdown(examCode: string): ExamCountdown | null {
  const now = new Date();
  const currentYear = now.getFullYear();

  // This pipeline only handles NEET. If called with a non-NEET code, return null.
  ...
  const target = { name: NEET_DATE.name, date: candidate };
