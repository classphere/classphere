...
export async function listAllInstitutes(): Promise<InstituteRow[]> {
  ...
  const { data: institutes, error: instErr } = await supabaseAdmin
    .from("institutes")
    .select("*")
...
export async function provisionInstitute(
  ...
  // ── 1. Duplicate guard ───────────────────────────────────────────────────
  const { data: existingUsers, error: dupCheckErr } = await supabaseAdmin
    .from("users")
    .select("id, role")
    .eq("email", adminEmail)
    .limit(1);
  ...
  const { data: newInst, error: insertErr } = await supabaseAdmin
    .from("institutes")
  ...
  if (insertErr) {
    ...
    try { await supabaseAdmin.from("users").delete().eq("id", newUserId); } catch (_) {}
    try { await supabaseAdmin.auth.admin.deleteUser(newUserId); } catch (_) {}
...
export async function getInstituteCRMStats(): Promise<{
  ...
  const { count: enterprisePlans } = await supabaseAdmin
    .from("institutes")
