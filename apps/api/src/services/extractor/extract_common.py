...
#     # Compact OCR/coaching layout with no space: "108.Identify...".
#     # Uppercase lookahead prevents decimals like 3.14 from matching.
...
def _best_chain(cands: list) -> list:
    """Pick the single best monotonic anchor chain out of `cands`. Rejects
    instruction lists / stray numbers by preferring, in order: longest chain,
    highest content quality (anchors followed by options/blanks/images), most
    annotated, most bold, earliest start."""
    if not cands:
        return []
...
def select_anchors(cands: list) -> list:
    """Pick anchors across possibly MULTIPLE monotonic runs.

    Multi-subject / multi-paper documents (e.g. JEE Advanced: Maths, then
    Physics, then Chemistry concatenated in one PDF) restart question
    numbering at 1 for each part. A single "longest chain" pick would only
    recognize the first part's numbering and silently dump every later
    part's content onto the tail of that part's last question (catastrophic
    -- can swallow 25+ pages into one block). To handle this we repeatedly
    find the best chain in whatever candidates remain, consume it, and
    continue the search strictly after it in document order. This still
    behaves exactly like the old single-chain logic for continuously
    numbered papers (NEET/JEE Main 1..N straight through) since there's only
    one run to find, but now also recovers each subsequent run when
    numbering resets.
    """
    if not cands:
        return []
    ...
    first_run = True
    while remaining:
        chain = _best_chain(remaining)
        if not chain:
            break

        # Guard against spurious "runs" made of isolated noise once we're
        # past the first (always-trusted) run: require either 2+ members or
        # a deterministically-annotated single anchor with question-like
        # content following it.
        if not first_run and len(chain) < 2 and not (chain[0][3] and chain[0][4]):
            break

        all_anchors.extend(chain)
        chain_line_idxs = {c[0] for c in chain}
        positions = [i for i, c in enumerate(remaining) if c[0] in chain_line_idxs]
        cut = max(positions) + 1
        remaining = remaining[cut:]
        first_run = False

    all_anchors.sort(key=lambda c: c[0])
    return all_anchors
...
def segment_questions(pages: list):
    ...
    for b in blocks:
        html = "\n".join(b["lines"])
        th = b["type_hint"] or type_hint_from_text(b["section_hint"] or "")
        # A qnum that doesn't continue the previous one (e.g. drops back to 1,
        # or otherwise fails to increase) marks the start of a new numbering
        # run -- typically a new subject/paper concatenated in the same PDF
        # (JEE Advanced-style Maths/Physics/Chemistry documents each restart
        # at Q.1). Downstream code should treat part_index changes as
        # authoritative subject/section boundaries rather than assuming one
        # continuous 1..N sequence across the whole document.
        if prev_qnum is not None and b["qnum"] <= prev_qnum:
            part_index += 1
        prev_qnum = b["qnum"]
        out.append({
            "qnum": b["qnum"],
