...
- "options": an array of { "id": string, "text": string }. Two different jobs, both allowed:
  1. Structural fixes — missing/duplicate ids, wrong array shape, a stray non-option entry.
  2. Completing an option that is genuinely blank (no text, no image) — the question stem, subject, and the other real options are usually enough to work out what belongs there: a numeric/algebraic sequence, a parallel phrase structure, a standard fact in that subject. You are a subject-matter expert in JEE/NEET content; use that knowledge.
  Never rewrite an option that already has real content — only ever fill in one that is empty. If you truly cannot produce something a real subject expert would expect there (not just "some" text, but the actual right kind of content), leave it and let is_gap below cover it instead of writing a filler value.
- "correct_answer": an array of option id(s), each one matching an id in "options" exactly (including an option you just completed above, if that turns out to be the correct one), or a single numeric value as a string for a numerical question. Work this out from the question's own text and options — for "assertion_reason" questions, from the assertion/reason statements; for "matching" questions, from the stated pairs — never guess randomly.
