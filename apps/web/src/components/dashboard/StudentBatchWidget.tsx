...
export function StudentBatchWidget({ batch }: StudentBatchWidgetProps) {
  if (!batch) {
    return (
      <div className="card bg-b-surface2 border border-s-stroke2 p-5 rounded-[20px] relative overflow-hidden group select-none">
        <div className="absolute inset-0 bg-gradient-to-br from-indigo-500/[0.02] to-purple-500/[0.02] pointer-events-none" />
        <div className="flex items-center gap-3">
          <div className="flex size-10 items-center justify-center rounded-[12px] bg-b-surface1 border border-s-stroke2 text-t-secondary">
            <RiTeamLine size={20} />
          </div>
          <div>
            <h3 className="font-bold text-sm text-t-primary">No Active Batch</h3>
            <p className="text-xs text-t-secondary">You are not currently enrolled in any batch.</p>
          </div>
        </div>
      </div>
    );
  ...
  return (
    <div className="card bg-b-surface2 border border-s-stroke2 p-5 rounded-[20px] relative overflow-hidden group select-none hover:border-t-secondary/20 transition-all duration-300">
      {/* Dynamic light gradient pattern */}
      <div className="absolute inset-0 bg-gradient-to-br from-primary-01/[0.03] to-primary-02/[0.03] pointer-events-none" />
      ...
      <div className="relative z-10 space-y-4">
        ...
        <div className="flex items-center justify-between">
          ...
          {/* Status Badge */}
          {isExpired ? (
            <span className="inline-flex items-center gap-1 px-2.5 py-0.5 rounded-full bg-red-500/10 text-red-500 font-bold text-[10px] uppercase tracking-wider border border-red-500/20">
              <RiErrorWarningLine size={11} /> Expired
            </span>
          ) : (
            <span className="inline-flex items-center gap-1 px-2.5 py-0.5 rounded-full bg-emerald-500/10 text-emerald-500 font-bold text-[10px] uppercase tracking-wider border border-emerald-500/20 animate-pulse">
        ...
        {/* Batch details */}
        <div>
          <h3 className="text-[17px] font-extrabold text-t-primary leading-[1.3] tracking-tight group-hover:text-primary-01 transition-colors">
            {batch.name}
          </h3>
        ...
        <div className="pt-2 border-t border-s-stroke2/50 grid grid-cols-2 gap-3 text-xs">
          ...
          <div className="space-y-0.5 text-right">
            ...
            {isExpired ? (
              <span className="text-red-500 font-extrabold">Batch Completed</span>
            ) : daysRemaining <= 30 ? (
              <span className="text-amber-500 font-extrabold animate-bounce">
                {daysRemaining} {daysRemaining === 1 ? "day" : "days"} left!
              </span>
            ) : (
              <span className="text-primary-02 font-extrabold">
