    ...
    <>
      ...
      <main className="mx-auto w-full max-w-[1560px] flex flex-col items-center pb-12 pt-6 gap-6 px-6 bg-transparent">
        ...
        <div className="flex flex-col xl:flex-row items-start gap-6 w-full">
          ...
          {/* Tests by Exam Type */}
          <div className="group relative card flex flex-col overflow-hidden p-6 md:p-8 rounded-lg bg-b-surface2 dark:bg-b-surface2 shadow-[0px_5px_1.5px_-4px_rgba(8,8,8,0.09),0px_6px_4px_-4px_rgba(8,8,8,0.05)] border border-s-stroke2/40 flex-1 min-w-0 h-full">
            <div className="box-hover" />
            <div className="relative z-10 flex flex-col items-start gap-6 w-full h-full">
              ...
              <div className="flex flex-col items-start gap-1 w-full">
                ...
                <span className="font-sans text-[14px] text-t-secondary">Distribution across all 845K tests conducted</span>
              </div>
                ...
                {examBreakdown.map((exam, i) => (
                  <div key={i} className="flex flex-col w-full gap-2">
                    <div className="flex flex-row justify-between items-center w-full">
                      <span className="font-sans text-[16px] font-semibold text-t-primary dark:text-t-primary">{exam.exam}</span>
                      <span className="font-sans text-[15px] font-medium text-t-secondary">
                        {exam.tests.toLocaleString()} <span className="font-semibold text-t-primary dark:text-t-primary ml-1">({exam.pct}%)</span>
                      </span>
                    </div>
                    
                    <div className="w-full h-2.5 bg-b-surface1 dark:bg-b-surface1 border border-s-stroke2/40 dark:border-s-stroke2/40 rounded-full relative overflow-hidden">
                      <div 
          ...
          <div className="group relative card flex flex-col overflow-hidden p-6 md:p-8 rounded-lg bg-b-surface2 dark:bg-b-surface2 shadow-[0px_5px_1.5px_-4px_rgba(8,8,8,0.09),0px_6px_4px_-4px_rgba(8,8,8,0.05)] border border-s-stroke2/40 w-full xl:w-[600px] shrink-0 h-full min-h-[354px]">
            <div className="box-hover" />
            ...
            <div className="relative z-10 flex flex-col items-start gap-6 w-full">
              <div className="flex flex-col items-start gap-1 w-full">
                ...
                <span className="font-sans text-[14px] text-t-secondary">Ranked by tests conducted this month</span>
              </div>

              <div className="flex flex-col items-start gap-2 w-full">
                {/* Table Header */}
                <div className="flex flex-row items-center w-full px-4 py-2 bg-b-surface1 dark:bg-b-surface1/60 rounded-lg border border-s-stroke2/20">
                  <span className="flex-1 font-sans text-[13px] font-semibold text-t-secondary uppercase tracking-[0.02em]">Institute</span>
                  <span className="w-24 font-sans text-[13px] font-semibold text-t-secondary uppercase tracking-[0.02em] text-right">Tests</span>
                  <span className="w-24 font-sans text-[13px] font-semibold text-t-secondary uppercase tracking-[0.02em] text-right">AI Tokens</span>
                </div>

                {topInstitutes.map((inst, i) => (
                  <div key={i} className="flex flex-row items-center w-full px-4 py-3 rounded-lg hover:bg-b-surface1 dark:hover:bg-b-surface1/40 transition-colors cursor-pointer">
                    <div className="flex-1 flex flex-row items-center gap-3">
                      <div className="w-8 h-8 rounded-lg bg-b-surface1 dark:bg-b-surface1 border border-s-stroke2/40 flex items-center justify-center font-sans font-bold text-[13px] text-t-primary dark:text-t-primary shrink-0">
                        {i + 1}
                      </div>
                      <span className="font-sans font-semibold text-[15px] text-t-primary dark:text-t-primary truncate">{inst.name}</span>
                    </div>
                    <span className="w-24 font-sans font-medium text-[15px] text-t-secondary text-right">{inst.tests.toLocaleString()}</span>
                    <span className="w-24 font-sans font-semibold text-[15px] text-t-primary dark:text-t-primary text-right">{inst.tokens}</span>
                  </div>
                ))}
              </div>
            </div>
          ...
          <div className="relative z-10 flex flex-col items-start gap-6 w-full">
            ...
            <div className="flex flex-row justify-between items-center w-full mb-2">
              <div className="flex flex-col items-start gap-1">
                ...
                <span className="font-sans text-[14px] text-t-secondary">142.8M tokens used this month across all generative workflows</span>
              </div>
              <div className="flex flex-row justify-center items-center px-4 py-2 gap-2 border border-s-stroke2/40 bg-[rgba(255,106,85,0.05)] rounded-lg">
                <span className="text-[14px] font-semibold leading-none">84% of monthly budget</span>
              </div>
            </div>
              ...
              {aiBreakdown.map((item, i) => {
                ...
                return (
                  <div key={i} className="flex flex-col w-full gap-3">
                    <div className="flex flex-row justify-between items-center w-full">
                      <span className="font-sans text-[16px] font-semibold text-t-primary dark:text-t-primary">{item.label}</span>
                      <div className="flex flex-row items-center gap-4">
                        <span className="font-sans text-[16px] font-semibold text-t-primary dark:text-t-primary">{item.value}M tokens</span>
                        <span className="font-sans text-[15px] font-bold text-t-secondary w-12 text-right">{pct}%</span>
                      </div>
                    </div>
                    
                    <div className="w-full h-3 bg-b-surface1 dark:bg-b-surface1 border border-s-stroke2/40 dark:border-s-stroke2/40 rounded-full relative overflow-hidden">
                      <div 
