  ...
  return <><Navbar title="Study Material" subtitle="Publish notes and resources to selected batches." breadcrumbs="TEST DEPARTMENT > STUDY MATERIAL" />
    <main className="mx-auto grid w-full max-w-screen-2xl gap-6 px-4 pb-12 md:grid-cols-[minmax(0,1fr)_22rem] md:px-8">
      <form onSubmit={publish} className="card p-5 sm:p-7"><h2 className="text-lg font-bold text-t-primary">Publish material</h2>
        ...
        {status && <p className="mt-4 text-sm text-t-secondary">{status}</p>}<button disabled={saving || !selectedBatchIds.length} className="mt-5 h-11 rounded-[10px] bg-shade-02 px-5 text-sm font-semibold text-white disabled:opacity-50">{saving ? "Publishing…" : "Publish material"}</button>
      </form>
