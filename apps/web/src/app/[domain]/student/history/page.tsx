...
type HistoryItem = {
  id: string; title: string; test_type: string; subject?: string; exam_code: string;
  score: number; max_score: number; percentage: number; submitted_at: string;
};
      ...
      <div className="relative z-10 flex flex-col gap-5 lg:flex-row lg:items-start lg:justify-between">
        <div className="min-w-0 flex-1">
          <div className="mb-3 flex flex-wrap items-center gap-2">
            ...
            <span className="text-[12px] font-sans font-semibold text-t-secondary tracking-wide">{item.submitted_at ? new Date(item.submitted_at).toLocaleDateString("en-IN", { day: "numeric", month: "short", year: "numeric" }) : ""}</span>
          </div>
          <h3 className="font-sans font-semibold text-[17px] leading-snug tracking-[-0.02em] text-t-primary">{item.title}</h3>
          {item.subject && <p className="text-[13px] font-sans text-t-secondary mt-0.5">{item.subject}</p>}
          <div className="mt-4 flex flex-wrap gap-1.5">
            <span className="text-[10px] font-sans font-bold px-2 py-0.5 border border-black/5 dark:border-white/5 bg-b-surface1 text-t-secondary rounded-[6px] uppercase tracking-wider">{item.score} / {item.max_score} marks</span>
            <span className="text-[10px] font-sans font-bold px-2 py-0.5 border border-black/5 dark:border-white/5 bg-b-surface1 text-t-secondary rounded-[6px] uppercase tracking-wider">{item.exam_code?.replace("-", " ").toUpperCase()}</span>
          </div>
        </div>
        <div className="flex shrink-0 items-start gap-4 lg:flex-col lg:items-end lg:text-right">
          <div>
            <div className={`font-sans text-[24px] font-semibold tracking-[-0.01em] leading-none ${pctColor}`}>{pct}%</div>
            <div className="text-[11px] font-sans font-bold text-t-secondary uppercase tracking-widest mt-1">Score</div>
          </div>
          <Link href={`/student/results/${item.id}`} className="block">
...
export default function HistoryPage() {
  ...
  const avgPct = history.length > 0 ? Math.round(history.reduce((s, h) => s + h.percentage, 0) / history.length) : 0;

  return (
    <>
      <Navbar title="Test History" subtitle="Your completed test sessions, performance record, and score analysis." breadcrumbs="Student > Test History" />
      <PageWrapper>
        <MetricGrid cols={3}>
          <MetricCard icon={<RiPlayLine size={18} />} label="Tests Completed" value={loading ? "—" : totalTests} badge="Total" badgeLabel="submitted attempts" />
          <MetricCard icon={<RiGitMergeLine size={18} />} label="Best Score" value={loading ? "—" : `${bestScore}%`} badge="Peak" badgeLabel="highest percentage" />
        ...
        {loading ? (
          ...
          <div className="flex flex-col gap-4">{history.map(item => <TestHistoryCard key={item.id} item={item} />)}</div>
        )}
      </PageWrapper>
    </>
  );
