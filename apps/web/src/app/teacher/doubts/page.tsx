...
export default function TeacherDoubtsPage() {
  ...
  return (
    <div className="flex flex-col min-h-screen bg-b-surface1 dark:bg-b-surface1/60 pb-10">
      ...
      {toastMessage && (
        <div className="fixed top-20 right-6 z-50 flex items-center gap-2.5 px-4 py-3 bg-shade-02 text-t-light text-xs font-sans font-semibold rounded-[10px] shadow-lg border border-s-stroke2/20">
        ...
        <div className="w-[1068px] h-[780px] bg-b-surface2 dark:bg-b-surface2 p-3 card flex flex-col gap-4 overflow-hidden relative">
          ...
          <div className="flex flex-row justify-between items-center w-full h-12 relative px-2.5 shrink-0">
            <div className="flex items-center gap-2">
              ...
              {pendingDoubts > 0 && (
                <span className="px-2 py-0.5 rounded-[10px] bg-primary-03/10 border border-primary-03/20 text-primary-03 text-[11px] font-bold">
            ...
            <div className="relative">
              <button
                ...
                className="flex flex-row justify-between items-center px-5 py-3 h-12 w-[190px] border-[1.5px] border-s-stroke2 dark:border-s-stroke2 rounded-[10px] bg-transparent text-left cursor-pointer transition-all hover:bg-b-surface1 dark:hover:bg-b-surface1"
              >
              ...
              {showDropdown && (
                <div className="absolute right-0 top-14 w-[190px] bg-b-surface2 dark:bg-b-surface2 border border-s-stroke2/40 rounded-[10px] shadow-lg z-25 overflow-hidden">
                  {groups.map(g => (
                    <div
                      key={g.id}
          ...
          <div className="flex flex-row items-center p-2 gap-4 w-full h-[135px] bg-b-surface1 dark:bg-b-surface1/20 border-[1.5px] border-s-stroke2/40 rounded-[10px] shrink-0">
            ...
            <div className="flex flex-col items-start py-3 px-8 gap-1 flex-grow h-[119px] bg-b-surface2 dark:bg-b-surface2 border-[1.5px] border-s-border dark:border-s-stroke2/20 rounded-[10px] shadow-[0px_0px_36px_-8px_rgba(0,0,0,0.05),0px_6px_4px_-4px_rgba(8,8,8,0.05),0px_5px_1.5px_-4px_rgba(8,8,8,0.09)]">
          ...
          <div className="flex-grow flex flex-row border border-s-stroke2/20 rounded-[10px] overflow-hidden min-h-0">
              ...
              <div className="p-3 border-b border-s-stroke2/10 bg-b-surface2 dark:bg-b-surface2">
                <div className="relative">
                  ...
                  <input
                    ...
                    className="w-full pl-9 pr-3 h-9 rounded-[10px] border border-s-stroke2/30 bg-b-surface1 dark:bg-b-surface1/60 text-xs font-sans focus:outline-none focus:border-t-primary transition-all font-semibold placeholder-t-secondary"
                  />
                </div>
              </div>
                ...
                {groups.map(group => {
                  ...
                  return (
                    <div
                      ...
                      className={`p-3 flex gap-3 cursor-pointer rounded-[10px] transition-all border ${isActive ? "bg-b-surface2 dark:bg-b-surface2 border-l-4 border-l-t-primary border-s-stroke2/20 shadow-xs" : "bg-transparent border-transparent hover:bg-b-surface2/50"}`}
                    >
            ...
            <div className="flex-1 flex flex-col bg-b-surface2 dark:bg-b-surface2 min-w-0">
              ...
              <div className="h-14 px-4 border-b border-s-stroke2/10 flex items-center justify-between shrink-0">
                ...
                {/* Filter tabs */}
                <div className="flex items-center gap-1 bg-b-surface1 dark:bg-b-surface1/40 border border-s-stroke2/20 rounded-[10px] p-1">
                  {(["all", "doubts"] as FilterTab[]).map(tab => (
                    <button
                      key={tab}
                      onClick={() => setFilterTab(tab)}
                      className={`px-3 h-7 rounded-[10px] text-[11px] font-sans font-semibold capitalize transition-all cursor-pointer border-0 ${filterTab === tab ? "bg-shade-02 text-t-light shadow-xs" : "bg-transparent text-t-secondary hover:text-t-primary"}`}
                    >
              ...
              <div className="flex-1 overflow-y-auto px-6 py-4 flex flex-col gap-3 bg-b-surface1/40 dark:bg-b-surface1/10 min-h-0">
                {activeMessages.map(msg => {
                  ...
                  return (
                    <div
                      key={msg.id}
                      className={`p-4 flex gap-4 rounded-[10px] transition-all relative border ${
                          ...
                          {msg.isVerified ? (
                            ...
                            !msg.isOwn && msg.role === "student" ? (
                              <button
                                ...
                                className="flex items-center gap-1.5 px-3 py-1 rounded-[10px] border border-primary-02/25 bg-primary-02/5 text-primary-02 text-[11px] font-sans font-bold hover:bg-primary-02/10 transition-all active:scale-95 cursor-pointer"
                              >
              ...
              <div className="p-3 bg-b-surface2 dark:bg-b-surface2 border-t border-s-stroke2/20 flex gap-2.5 items-center shrink-0">
                ...
                <div className="flex-grow">
                  <input
                    ...
                    className="w-full pl-4 pr-3 h-9 rounded-[10px] border border-s-stroke2/30 bg-b-surface1 dark:bg-b-surface1/60 text-xs font-sans focus:outline-none focus:border-t-primary transition-all font-semibold placeholder-t-secondary text-t-primary"
                  />
                </div>
                <button
                  onClick={handleSend}
